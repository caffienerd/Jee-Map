...
const Notes = (() => {
  ...
  function invalidate() { _notes = null; }
  ...
  function _bindCardEvents(canEdit, viewingUserId) {
    document.querySelectorAll('.note-card').forEach(card => {
      ...
      card.addEventListener('touchstart', () => {
        ...
        _pressTimer = setTimeout(() => {
          ...
          if (canEdit) {
            ...
            if (note) { card.classList.add('note-card--pressed'); _openModal(note, viewingUserId); }
          }
        }, 500);
      }, { passive: true });
      ...
      card.addEventListener('touchmove', () => clearTimeout(_pressTimer), { passive: true });
  ...
  function _openViewModal(note, viewingUserId, canEdit) {
    ...
    requestAnimationFrame(() => document.getElementById('notesViewModal')?.classList.add('is-open'));
    ...
    document.addEventListener('keydown', function escHandler(e) {
      if (e.key === 'Escape') { closeView(); document.removeEventListener('keydown', escHandler); }
    });
  ...
  function _openModal(note, viewingUserId) {
    ...
    const priOptions = [0,1,2,3,4,5].map(p => {
      const cfg  = PRI[p];
    ...
    const subjOptions = [null, 'physics', 'chemistry', 'maths', 'general'].map(s => {
      const cfg = SUBJ[s];
    ...
    document.querySelectorAll('.mpri-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        document.querySelectorAll('.mpri-btn').forEach(b => b.classList.remove('is-sel'));
    ...
    document.querySelectorAll('.msubj-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        document.querySelectorAll('.msubj-btn').forEach(b => b.classList.remove('is-sel'));
        btn.classList.add('is-sel');
      });
    });

    // ── Save
    document.getElementById('nmodalSave')?.addEventListener('click', async () => {
    ...
    // ── Delete
    document.getElementById('nmodalDel')?.addEventListener('click', async () => {
      if (!note) return;
