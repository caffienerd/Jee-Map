...
Deno.serve(async (req) => {
  try {
    ...
    const actorName = actorId === USERS.rishit ? 'Rishit' : 'Vedanta';
    const otherId   = actorId === USERS.rishit ? USERS.vedanta : USERS.rishit;
    ...
    console.log(`[notify] fetching tokens — notifyAll=${notifyAll}, otherId=${otherId}`);

    let query = db.from('device_tokens').select('token, user_id, is_anonymous');

    if (!notifyAll) {
      // Private note: only the other editor, not guests
      query = query.eq('user_id', otherId);
    }

    const { data: tokens, error } = await query;

    console.log(`[notify] token fetch result: count=${tokens?.length ?? 0}, error=${error?.message ?? 'none'}`);
    ...
    console.log(`[notify] after filtering author: ${filtered.length} tokens remain`);
    ...
    const results = await Promise.all(
      filtered.map(async ({ token }) => {
        const result = await sendFCM(token, title, body, fcmData, accessToken, projectId);
        ...
        if (errorCode === 'UNREGISTERED' || errorCode === 'INVALID_ARGUMENT') {
          console.log(`[notify] removing stale token: ${token.slice(0, 20)}...`);
    ...
    console.log(`[notify] done — ${succeeded} sent, ${failed} stale tokens cleaned`);
